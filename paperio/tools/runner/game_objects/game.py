...
class Game:
    RESULT_LOCATION = os.environ.get('GAME_LOG_LOCATION', None)

    BASE_DIR = os.path.dirname(RESULT_LOCATION) if RESULT_LOCATION else None

    VISIO_LOCATION = os.path.join(BASE_DIR, 'visio.gz') if BASE_DIR else None
    SCORES_LOCATION = os.path.join(BASE_DIR, 'scores.json') if BASE_DIR else None
    DEBUG_LOCATION = os.path.join(BASE_DIR, '{}') if BASE_DIR else None
